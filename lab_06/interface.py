...
class MainWindowClass(tk.Frame):
    ...
    def add_buttons(self):
        ...
        seed_point = cfg.create_entry(self.frame_buttons, FONT, 2, 0, PADX, PADY, tk.N)
        cfg.create_button(self.frame_buttons, 'Закрасить фигуру', FONT, 3, 0, PADX, PADY, 1, 'WE', lambda:
                          alg.algorithm_seed(self.canvas, self.seed.get(), False if self.mode.get() == 0 else True))

        cfg.create_radiobutton(self.frame_buttons, self.mode, 0, 'Без задержки', FONT, 4, 0, PADX, PADY, 'WN')
        cfg.create_radiobutton(self.frame_buttons, self.mode, 1, 'C задержкой', FONT, 5, 0, PADX, PADY, 'WN')
    ...
    def count_time(self):
        time = alg.algorithm_seed(self.canvas, self.seed)
